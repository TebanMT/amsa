...
func getUnitsHandler(ctx context.Context, request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	useCaseUnit := use_cases.NewUnitUseCase(unitRepo)

	categories, err := useCaseUnit.GetAllUnitsByCompany(ctx, idCompany)
	if err != nil {
