...
func getProductsHandler(ctx context.Context, request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {

	fmt.Printf("Procesando solicitud: %s\n", request)
	idCompany, _ := strconv.Atoi(request.QueryStringParameters["idCompany"])
	idType, _ := strconv.Atoi(request.QueryStringParameters["idType"])
	...
	products, err := createProductUseCase.GetAllProductsByCompanyAndType(ctx, idCompany, idType)
	if err != nil {
