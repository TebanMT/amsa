...
func createClientHandler(ctx context.Context, request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	createClientUseCase := use_cases.NewCreateClientUseCase(clientRepo)

	// Crear el cliente
	if err := createClientUseCase.Execute(ctx, &client); err != nil {
...
func main() {
	lambda.Start(createClientHandler)
}
