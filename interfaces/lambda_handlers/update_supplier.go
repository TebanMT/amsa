...
func updateSupplierHandler(ctx context.Context, request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	// update proveedor
	if err := createSupplierUseCase.UpdateSupplier(ctx, &supplier); err != nil {
...
func main() {
	lambda.Start(updateSupplierHandler)
}
